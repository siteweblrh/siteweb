// Home page — Desktop (1440 wide) and Mobile (390 wide)
// Shares state for Gazon/Salle toggle via prop.

function NavLink({ children, active = false, white = false }) {
  return (
    <div style={{
...
function SeasonToggle({ mode, setMode, size = 'md' }) {
  const isLg = size === 'lg';
...
function CTAButton({ children, variant = 'red', size = 'md' }) {
  const isLg = size === 'lg';
  const palettes = {
    red:   { bg: LRH.red,  fg: '#fff',     hover: LRH.redDeep },
    gold:  { bg: LRH.gold, fg: LRH.navy,   hover: '#E0A810' },
    navy:  { bg: LRH.navy, fg: '#fff',     hover: LRH.navyDeep },
...
// ── HEADER ─────────────────────────────────────────────────────
function DesktopHeader({ mode, setMode }) {
  return (
    <div style={{ background: '#fff', borderBottom: '1px solid ' + LRH.hair }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 18 }}>
          ...
          <span style={{ color: LRH.gold }}>Espace Clubs</span>
        </div>
      </div>
      {/* Main row */}
      <div style={{
...
function Card({ children, dark = false, style = {}, hoverShadow = true }) {
  return (
    <div style={{
...
function CardHeader({ kicker, meta }) {
  return (
...
function CardHeaderDark({ kicker, meta }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
  ...
  return (
    <Card style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <div style={{ position: 'relative', height: 220 }}>
        ...
        {/* Sponsor lozenge — native integration */}
        <div style={{
...
// ── NEWS ──────────────────────────────────────────────────────
function NewsSection({ mode }) {
  const items = [
    { tag: 'Reportage', kicker: '03 min', title: 'Au cœur de la 7<sup>ème</sup> nuit du hockey péï', desc: 'Trois cents licenciés réunis à Saint-Paul pour célébrer une saison Gazon qui défie les pronostics.', tone: 'sun', sponsor: null, big: true },
    { tag: 'Tactique',  kicker: '06 min', title: 'Pourquoi le pressing haut de Tampon casse les défenses du Nord', desc: 'Décryptage du système 1-4-3 qui a propulsé les rouges en tête.', tone: 'turf', sponsor: 'OUEST TV' },
    { tag: 'Formation', kicker: '04 min', title: 'Nouvelle académie U15 — les inscriptions sont ouvertes', desc: '24 places, 4 sites, gratuité pour les licenciés.', tone: 'paper', sponsor: null },
  ];
  return (
...
function NewsCard({ tag, kicker, title, desc, tone, sponsor, big }) {
  return (
    <Card style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
...
// ── COMPETITIONS STRIP ─────────────────────────────────────────
function CompetitionsStrip({ mode }) {
  return (
...
// clubName is exported from tokens.jsx

function ChipDark({ children, active = false }) {
  return (
    <div style={{
...
// ── ASSEMBLY ───────────────────────────────────────────────────
function HomeDesktop({ mode, setMode }) {
  return (
...
Object.assign(window, {
  HomeDesktop, SeasonToggle, CTAButton, Card, ClubCrest,
  CardHeader, CardHeaderDark, Stat, SectionHeading, ChipDark,
  MatchChocGlass, ImageSlot, LrhLockup, NavLink, clubName,
});
