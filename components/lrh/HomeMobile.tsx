...
function MobileHero({ mode }: { mode: 'gazon' | 'salle' }) {
...
function MobileBento({ mode }: { mode: 'gazon' | 'salle' }) {
  return (
...
function MobileCompetitions({ mode }: { mode: 'gazon' | 'salle' }) {
  return (
...
function MobileNews() {
  const items = [
    { tag: 'Reportage', t: 'Au cœur de la 7<sup>ème</sup> nuit du hockey péï', tone: 'sun' as const, mins: '03 min', big: true },
    { tag: 'Tactique',  t: 'Le pressing haut de Tampon, décryptée.',           tone: 'turf' as const, mins: '06 min', sponsor: 'OUEST TV' },
    { tag: 'Formation', t: 'Académie U15 — inscriptions ouvertes',             tone: 'paper' as const, mins: '04 min' },
  ];
  return (
...
export function HomeMobile({ mode, setMode }: { mode: 'gazon' | 'salle', setMode: (m: 'gazon' | 'salle') => void }) {
  return (
